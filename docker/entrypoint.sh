...
wait_for_db() {
  db_address=$(getent hosts db | awk '{ print $1 }')
  counter=0
  log "Connecting to db at $db_address"
  while ! curl --silent db:3306 >/dev/null; do
    counter=$((counter+1))
    if [ $counter == 30 ]; then
      log "Error: Couldn't connect to db."
      exit 1
    fi
    log "Trying to connect to db at $db_address. Attempt $counter."
    sleep 5
  done
}
